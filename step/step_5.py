...
class TrajectoryStep:
    ...
    def extract_country_crude_oil(self, port_shp_name, deadweight_db, deadweight_table, import_file_name,
        ...
        self.export_country_info(before, source_port_name, port_service, deadweights, export_writer)
    ...
    def export_country_info(self, info, port_name, port_service, deadweights, writer, ):
        if not self.judge_full_load(info):
            return

        port = port_service.get_port_by_name(port_name)
        deadweight = deadweights.get_deadweight_by_mmsi(info[TrajectoryStep.MMSI_INDEX])
        writer.writerow([port.country, info[TrajectoryStep.LON_INDEX], info[TrajectoryStep.LAT_INDEX],
                         info[TrajectoryStep.DATE_INDEX], deadweight])
...
if __name__ == '__main__':
    ...
    clean_file_name = r"D:\graduation\data\step_result\step4\trajectory_clean.csv"
    ...
    import_file_name = r"D:\graduation\data\step_result\step5\country_import_with_position.csv"
    export_file_name = r"D:\graduation\data\step_result\step5\country_export_with_position.csv"
    file_header = ["country", "date", "longitude", "latitude", "deadweight"]

    trajectory = TrajectoryStep(clean_file_name)
    trajectory.extract_country_crude_oil(port_name, deadweight_db, deadweight_table, import_file_name, export_file_name,
                                         file_header)
